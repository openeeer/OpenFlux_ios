...
func NewRawSocketEndpoint(nicID tcpip.NICID) (*RawSocketEndpoint, error) {
	...
	copy(ep.localIP[:], net.ParseIP(getLocalIP()).To4())
...
func (e *RawSocketEndpoint) readLoop() {
	...
	for {
		...
		info, valid := network.ParseTCP(buf[:n])
		if !valid {
			continue
		}
		ipLen := int(buf[0]&15) * 4
		flags := info.Flags
		if [4]byte(buf[16:20]) == e.localIP {
			dstPort := uint16(buf[ipLen+2])<<8 | uint16(buf[ipLen+3])
			...
			pktCopy[10] = 0
			pktCopy[11] = 0
			ipChecksumVal := network.IPChecksum(pktCopy[:ipLen])
...
func (e *RawSocketEndpoint) WritePackets(pkts stack.PacketBufferList) (int, tcpip.Error) {
	...
	for _, pkt := range pkts.AsSlice() {
		view := pkt.ToView()
		ipPacket := append([]byte(nil), view.AsSlice()...)
		view.Release()
		if len(ipPacket) < 40 {
			continue
		}

		if _, valid := network.ParseTCP(ipPacket); !valid {
			continue
		}
		pktCopy := ipPacket
		...
		pktCopy[10] = 0
		pktCopy[11] = 0
		ipChecksumVal := network.IPChecksum(pktCopy[:int(pktCopy[0]&15)*4])
		pktCopy[10] = byte(ipChecksumVal >> 8)
		pktCopy[11] = byte(ipChecksumVal & 0xFF)
