...
// EncodeLegacyPayload and DecodeLegacyPayload retain wire compatibility with
// the original per-packet compression wrapper while the Yandex transport moves
// compression to batch boundaries.
func EncodeLegacyPayload(data []byte) []byte { return compress(data) }

func DecodeLegacyPayload(data []byte) ([]byte, error) {
