...
type TransportConfig struct {
	...
	// Yandex Docs transport tuning. These values intentionally live in the
	// common config so callers can configure a transport without type casts.
	YandexLegacy          bool
	YandexBatchDelay      time.Duration
	YandexBatchBytes      int
	YandexBatchPackets    int
	YandexQueueTimeout    time.Duration
	YandexStats           bool
	YandexStatsInterval   time.Duration
	YandexCompressBatches bool
}
...
func DefaultConfig() TransportConfig {
	return TransportConfig{
		...
		YandexBatchDelay:     time.Millisecond,
		YandexBatchBytes:     16 * 1024,
		YandexBatchPackets:   16,
		YandexQueueTimeout:   50 * time.Millisecond,
		YandexStatsInterval:  5 * time.Second,
	}
}
